...
file.edit("~/.Renviron")
# Paste this directly into the .Renviron file that opens after the line above
# GBIF_USER=username
# GBIF_PWD=password
# GBIF_EMAIL=email

# Verify login into GBIF (should print user & email)
...
# Making assemblages
empirical_assemblages <- occ_df %>%
  select(cell_id, genus, species, country = countryCode, locality) %>% 
  distinct(cell_id, genus, species, .keep_all = TRUE) %>%
  group_by(cell_id) %>%
  mutate(sp_rich = n_distinct(paste(genus, species))) %>%
  ungroup() %>%
  # Separating genus and species names
  separate(
    species,
    into = c("drop", "species"),
    sep = " ",
  ...
  select(-drop) %>% 
  # Filter out occurrences where genus and/or species are blank
  filter(!is.na(species)) %>% 
  filter(genus != "")

# Filter for grids with n species (optional)
n3_empirical <- empirical_assemblages %>% 
  filter(sp_rich == "3")
