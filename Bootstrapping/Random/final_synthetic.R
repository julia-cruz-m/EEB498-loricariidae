# Morphospace Metrics for Random Assemblages
...
# No unique genera
  ...
  N_rep <- 1000
  richness_vals <- 2:26
  total_reps <- length(richness_vals) * N_rep
  
  boot_list <- vector("list", total_reps)
  
  # sampling function
  random_species <- function(data, N) {
    selected_genera <- sample(unique(data$genus), N, replace = TRUE)
    
    sampled_genera <- tibble(genus = selected_genera)
    
    sampled <- sampled_genera %>%
      rowwise() %>%
      mutate(species = sample(data$species[data$genus == genus], 1)) %>%
      ungroup() %>%
      left_join(data, by = c("genus", "species")) %>%
      mutate(sp_rich = N)
    
    return(sampled)
  }
  
  # Fill list sequentially
  counter <- 1
  
  for (N in richness_vals) {
    for (i in 1:N_rep) {
      boot_list[[counter]] <- random_species(pca, N)
      counter <- counter + 1
    }
  }
 ...
 #   mutate(replicate = as.integer(replicate))
  ...
  sp_rich_vec <- sapply(boot_list, function(x) unique(x$sp_rich))
  
  # Adding centroid, CD, and hypervolume to original NND dataframe
  nnd_df$replicate <- as.integer(nnd_df$replicate)
  n_centroid$replicate <- as.integer(n_centroid$replicate)
  n_cd$replicate <- as.integer(n_cd$replicate)
  hv_df$replicate <- as.integer(hv_df$replicate)
  
  final_df <- nnd_df %>%
    left_join(n_centroid, by = "replicate") %>%
    left_join(n_cd, by = "replicate") %>% 
    left_join(hv_df, by = "replicate") %>% 
    mutate(sp_rich = sp_rich_vec[replicate])
...
write.csv(final_all, "final_all_1.csv")
